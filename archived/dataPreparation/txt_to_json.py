...
def convert_txt_to_json(file_name:str='', folder_name:str='', output_folder:str='ontology_outputs_json/', content:Optional[str]=''):
    """
    ...
    if not content and not (file_name and folder_name):
        raise ValueError("Either `content` or both `file_name` and `folder_name` must be provided")
    ...
    if match:
        ...
        print("Success: Created and stored the json in the json file")
    else:
...
if __name__ == "__main__":
    ...
    json_format = convert_txt_to_json(file_name=file_name, folder_name=folder_name, output_folder=output_folder)